...
: m_time(0)
{
	...
	m_bassinstfactory.LoadFile("jp_deep_bass_01.wav");
...
bool CSynthesizer::Generate(double * frame)
{
	...
	while (m_currentNote < (int)m_notes.size())
	{
		...
		else if (note->Instrument() == L"WaveBass")
		{
			//m_bassinstfactory.LoadFile("jp_deep_bass_03.wav");
			m_bassinstfactory.SetNote(note);
			instrument = m_bassinstfactory.CreateInstrument();
		} else if (note->Instrument() == L"WaveSines")
		{
			m_wavesinesfactory.SetNote(note);
			instrument = m_wavesinesfactory.CreateInstrument();
		}
	...
	for (list<CInstrument *>::iterator node = m_instruments.begin(); node != m_instruments.end();)
	{
		...
		if (instrument->Generate())
		{
			...
			double nframein[2];
			double nframe[2];
			nframein[0] = instrument->Frame(0);
			nframein[1] = instrument->Frame(1);
;
			
			m_noisegate.Process(nframein, nframe);
	...
	if (m_beat > m_beatspermeasure)
	{
		...
		m_measure++;
		//m_noisegate.SetMeasure(m_measure);
		m_noisegate.SetThreshold(0.0);
	}
	//
